...
def _v3_dirs_for_lang(lang: str) -> Tuple[Optional[str], Optional[str]]:
    """
    ...
    logger.info(f"[models] {lang} v3 det_dir={'OK' if det_dir else 'MISS'} "
                f"rec_dir={'OK' if rec_dir else 'MISS'} base={PWHL}")
    ...
    def _load_one(self, lang: str):
        ...
        try:
            ...
            self.models[lang] = PaddleOCR(
                ...
                use_gpu=False,
                det_batch_num=1, rec_batch_num=1, cls_batch_num=1,
...
def ocr_lines(img: np.ndarray, lang: str) -> List[Tuple[str, float]]:
    ...
    for item in blocks or []:
        ...
        info = item[1]
        if not isinstance(info, (list,tuple)) or len(info) < 2: continue
        txt = str(info[0] or "").strip()
        try: conf = float(info[1]) if info[1] is not None else -1.0
        except: conf = -1.0
        if txt: out.append((txt, conf))
    return out
...
def _digits_loose_to_float(token: str) -> Optional[float]:
    ...
    t = (token.upper().replace('O','0').replace('Q','0').replace('D','0')
...
def parse_banner_time_robust(s: str) -> Optional[float]:
    ...
    for m in re.finditer(r'([0-9OQDBZGISL\,\.]{3,12})', s):
        ...
        j, score = m.start(), 0
        if i != -1 and 0 <= (j - i) <= 80: score += 2
        if re.search(r'(SEC|초)', s[m.end(): m.end()+8]): score += 1
...
_GENERIC_ASCII = {
    "MISSION", "COMPLETE", "TIME", "CLEAR", "PLAYER",
    "SPLIT", "LEVEL", "EASY", "EXTREME", "HARD",
    "MEDIUM", "NORMAL", "TOP", "SEC"
}
...
def extract_name(img_bl: np.ndarray, img_bl_alt: np.ndarray, img_ban: np.ndarray,
                 img_ban_white: np.ndarray, img_ban_bin: np.ndarray,
                 text_ban_en: str, text_bl_en: str, text_tr_en: str) -> Optional[str]:
    """
    Stratégie :
...
def extract(payload: B64):
    ...
    ban_bin   = cv2.adaptiveThreshold(ban_gray, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C,
    ...
    text_tlW_en   = join_lines(ocr_lines(tl_white_mask, "en")) if tl_white_mask is not None else ""
    text_tlC_en   = join_lines(ocr_lines(tl_cyan_mask,  "en")) if tl_cyan_mask  is not None else ""

    sec  = parse_banner_time_robust(text_ban_en)
    if sec is None:
        _m = re.search(r"\b(\d{1,4}[.,]\d{2})\s*SEC", (text_tr_en or "").upper())
        if _m:
            try: sec = float(_m.group(1).replace(",", "."))
            except: sec = None

    code = extract_code(text_tl_en, text_tlW_en, text_tlC_en)

    name = extract_name(
        img_bl=bl_n, img_bl_alt=bl, img_ban=ban,
        img_ban_white=ban_white, img_ban_bin=ban_bin,
        text_ban_en=text_ban_en, text_bl_en=text_bl_en, text_tr_en=text_tr_en
    )
    ...
    return {
        "extracted": {
            ...
            "texts": {
                "topLeft": text_tl_en,
                "topLeftWhite": text_tlW_en,
                "topLeftCyan": text_tlC_en,
                "banner": text_ban_en,
                "topRight": text_tr_en,
